...
TextRecognizer::TextRecognizer(Config cfg){
    std::string instanceName{"Rec model"};
    ...
    if (cfg.rec_useTRT){
        ...
        tensorrt_options->device_id = cfg.rec_device_id;
    ...
    if (cfg.rec_useCUDA){
        ...
        cudaOptions.device_id = cfg.rec_device_id;
        session_options.AppendExecutionProvider_CUDA(cudaOptions);
    }
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    session = Ort::Session(env, cfg.rec_model_path.c_str(), session_options);
    env.release();

    // create pre and post process
    postprocess = CTCLabelDecode(cfg);
    ...
    size_t input_tensor_size_single = 48 * cfg.rec_imgsz * 3;
    ...
    for (int i=0; i<cfg.rec_batch_size; i++){
        ...
        std::vector<int64_t> inputTensorShape {i+1, 3, 48, cfg.rec_imgsz};
    ...
    std::cout << "Done Init Text Recognizer" << std::endl;

    this->cfg = cfg;

}

std::vector<std::pair<std::string, float>> TextRecognizer::rec(std::vector<cv::Mat> list_imgs){
    auto timeStart = std::chrono::high_resolution_clock::now();

    int batch_size = list_imgs.size();
    size_t input_tensor_size = 3*48*cfg.rec_imgsz*batch_size;
    std::vector<int64_t> inputTensorShape {batch_size, 3, 48, cfg.rec_imgsz};
    std::vector<float> batch_input = Preprocess::preprocessRec(list_imgs, cfg);
